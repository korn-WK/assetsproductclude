...
const googleAuthCallback = (req, res) => {
  if (req.user) {
    ...
    res.cookie("jwt_token", token, {
      ...
      maxAge: 60 * 60 * 1000, // Cookie expires in 24 hours (match token expiry or session expiry)
    ...
    const isSuperAdmin =
      ...
      req.user.email === "admin@mfu.ac.th" ||
      req.user.email?.toLowerCase().includes("superadmin");
    const isAdmin =
      originalUserRole === "Admin" || originalUserRole === "admin";
    let redirectUrl;
    if (isSuperAdmin) {
      redirectUrl = `${process.env.CLIENT_URL}/admin/asset-management`;
    } else if (
      isAdmin ||
      originalUserRole === "User" ||
      originalUserRole === "user"
    ) {
...
const authorizeRoles = (...roles) => {
  return (req, res, next) => {
    ...
    const hasAllowedRole = roles.some(
      (role) => hashRole(role) === userRoleHash
    );
